    ...
    if os.path.exists(f_name):
        with open(f_name, "r", encoding="utf-8") as f:
            ...
            for s_id, story in data.items():
                ...
                categories_map[cat].append((s_id, story["title"], f_name))

print("--- HORROR & MYSTERY (Korku & Gizem) ---")
for s_id, title, src in sorted(categories_map['horror_mystery'], key=lambda x: x[1]):
    print(f"  - {title} (ID: {s_id}, Src: {src})")

print("\n--- FAIRY TALES & KIDS (Masallar & Çocuk) ---")
for s_id, title, src in sorted(categories_map['kids_fables'], key=lambda x: x[1]):
    print(f"  - {title} (ID: {s_id}, Src: {src})")

print("\n--- CLASSICS & ADVENTURE (Dünya Klasikleri) ---")
for s_id, title, src in sorted(categories_map['classics_adventure'], key=lambda x: x[1]):
    print(f"  - {title} (ID: {s_id}, Src: {src})")
