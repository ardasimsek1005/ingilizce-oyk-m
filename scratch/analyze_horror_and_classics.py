...
files = ["horror_stories_data.json", "classics_stories_data.json"]
...
print("--- horror_stories_data.json ---")
with open("horror_stories_data.json", "r", encoding="utf-8") as f:
    data = json.load(f)
    for s_id, story in data.items():
        cat = get_category(s_id)
        print(f"  - {story['title']} (ID: {s_id}) -> {cat}")

print("\n--- classics_stories_data.json ---")
with open("classics_stories_data.json", "r", encoding="utf-8") as f:
    data = json.load(f)
    for s_id, story in data.items():
        cat = get_category(s_id)
