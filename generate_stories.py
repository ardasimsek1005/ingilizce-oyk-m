...
for idx, story in enumerate(STORIES_TO_GENERATE):
    ...
    sys_instruction = f"You are a professional children's literary author and language teacher. You write stories for English learners at the CEFR {story['level']} level. Your target word count for the entire story is 1500 to 2000 words. You will write the story across 5 chapters. Crucial rule: The story plot, characters, and sequence of events MUST strictly match the original classic plot of the story '{story['title']}' by {story['author']} without any modifications."
    ...
    while chapter <= 5:
        ...
        if result is None:
            ...
            print("  Persistent rate limit or error encountered. Sleeping for 300 seconds (5 minutes) before retrying this chapter...")
            time.sleep(300)
            # Re-try same chapter by continuing loop without incrementing chapter index
            continue
